...
@EventBusSubscriber
public class StrawberrymcModVariables {
	public static final DeferredRegister<AttachmentType<?>> ATTACHMENT_TYPES = DeferredRegister.create(NeoForgeRegistries.Keys.ATTACHMENT_TYPES, StrawberrymcMod.MODID);

	@SubscribeEvent
	public static void init(FMLCommonSetupEvent event) {
		StrawberrymcMod.addNetworkMessage(SavedDataSyncMessage.TYPE, SavedDataSyncMessage.STREAM_CODEC, SavedDataSyncMessage::handleData);
	}
	...
	public static class WorldVariables extends SavedData {
		public static final SavedDataType<WorldVariables> TYPE = new SavedDataType<>(Identifier.parse("strawberrymc:worldvars"), level -> new WorldVariables(), level -> CompoundTag.CODEC.xmap(tag -> {
		...
		public BlockState Radiation = Blocks.AIR.defaultBlockState();

		public void read(CompoundTag nbt, HolderLookup.Provider lookupProvider) {
			Radiation = NbtUtils.readBlockState(lookupProvider.lookupOrThrow(BuiltInRegistries.BLOCK.key()), nbt.getCompoundOrEmpty("Radiation"));
		}

		public CompoundTag save(CompoundTag nbt, HolderLookup.Provider lookupProvider) {
			nbt.put("Radiation", NbtUtils.writeBlockState(Radiation));
			return nbt;
		}
	...
	public static class MapVariables extends SavedData {
		public static final SavedDataType<MapVariables> TYPE = new SavedDataType<>(Identifier.parse("strawberrymc:mapvars"), level -> new MapVariables(), level -> CompoundTag.CODEC.xmap(tag -> {
			MapVariables instance = new MapVariables();
			instance.read(tag, level.registryAccess());
			return instance;
		}, instance -> instance.save(new CompoundTag(), level.registryAccess())));
		boolean _syncDirty = false;
		public double RadiationCount = 0;

		public void read(CompoundTag nbt, HolderLookup.Provider lookupProvider) {
			RadiationCount = nbt.getDoubleOr("RadiationCount", 0);
		}

		public CompoundTag save(CompoundTag nbt, HolderLookup.Provider lookupProvider) {
			nbt.putDouble("RadiationCount", RadiationCount);
			return nbt;
		}
	...
	public record SavedDataSyncMessage(int dataType, SavedData data) implements CustomPacketPayload {
		public static final Type<SavedDataSyncMessage> TYPE = new Type<>(Identifier.fromNamespaceAndPath(StrawberrymcMod.MODID, "saved_data_sync"));
